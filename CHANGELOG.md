...
- Confirmed the three Workbench distributions (`axi-workbench-web-dist`, `axi-workbench-mobile-dist`, `axi-workbench-desktop-dist`) and `openclaw-gateway` are intentional one-sided registrations (graph-only / registry-only respectively) per ADR-005/006 and the task-execution-routing/v1 contract.
