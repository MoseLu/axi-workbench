...
# Stream chunk types for streaming output
...
class StreamChunk(BaseModel):
    ...
    type: str  # StreamChunkType values
    step: int | None = None
...
5. Do not include "user" related content, only summarize the Agent's execution process"""
    ...
    async def run_stream(self, cancel_event: Optional[asyncio.Event] = None):
        """Execute agent loop with streaming output.

        This is a generator that yields StreamChunk objects for each event
        during agent execution. Use this for real-time UI updates.

        Args:
            cancel_event: Optional asyncio.Event that can be set to cancel execution.

        Yields:
            StreamChunk objects representing different events during execution.
        """
        ...
        while step < self.max_steps:
            ...
            if self._check_cancelled():
                self._cleanup_incomplete_messages()
                yield StreamChunk(
                    type=StreamChunkType.CANCELLED,
                    step=step + 1,
            ...
            for tool_call in response.tool_calls:
                ...
                # Yield tool result
                yield StreamChunk(
                    type=StreamChunkType.TOOL_CALL_RESULT,
                    step=step + 1,
                    tool_call_id=tool_call_id,
                    tool_name=function_name,
