...
# Workspace Dev Services —— 代理契约

> 本档案是 **Workspace Dev Services**（工作区路径：`/Volumes/code/workspace/dev-services.config.json`）在 Axi Docs 中的代理契约。
> 它不取代项目根目录的 `AGENTS.md`。项目级规则始终以项目根目录的 `AGENTS.md` 为准；本文件仅记录 Axi Docs 如何*呈现*该项目。

## 阅读顺序

1. 本文件（档案）。
2. `docs/content/{en,zh}/projects/workspace-dev-services/README.md`（档案摘要）。
3. 项目根目录的 `AGENTS.md`：`/Volumes/code/workspace/dev-services.config.json/AGENTS.md`。
4. 项目根目录的 `README.md`：`/Volumes/code/workspace/dev-services.config.json/README.md`。

## 边界

- Axi Docs 将本项目视为**只读的内容来源**。
- Axi Docs 绝不编辑 `/Volumes/code/workspace/dev-services.config.json` 下的任何文件。
