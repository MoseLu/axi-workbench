...
| `axi-feishu-codex-bridge` | 未评估 | 低 | partial | docs:partial<br>handoff:verified<br>... | 补齐 TODO.md，让待办和里程碑可审计。<br>补齐 MILESTONE.md，让待办和里程碑可审计。<br>... |
...
| `ielts-vocab` | 未评估 | 低 | partial | docs:partial<br>handoff:verified<br>... | 补齐 TODO.md，让待办和里程碑可审计。<br>补齐 VERIFICATION.md：verify 命令已声明但缺沉淀位置。运行 `node bin/axi-todo.mjs verify-log` 或 workspace-verification.mjs 生成。 |
