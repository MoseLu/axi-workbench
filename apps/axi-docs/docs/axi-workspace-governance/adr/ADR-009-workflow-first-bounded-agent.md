# ADR-009: Workflow-first，Agent 仅作为受限探索执行器

> **Renumbered 2026-09-24** from `ADR-005-workflow-first-bounded-agent.md`
> to `ADR-009-workflow-first-bounded-agent.md` to resolve an ID collision with
> `ADR-005-agent-bff-ownership.md` (same numeric slot, unrelated topic).
> Status, decision, decision driver, and date are unchanged; the renumber is a
> bookkeeping fix recorded here so the historical filename and links in
> archived `docs/state/` material still trace back to the same decision.
