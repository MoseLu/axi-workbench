...
export default function ProfilePage() {
  ...
  const beginPairing = async () => {
    ...
    try {
      ...
      setPairingMessage('无法启动配对：控制面不可用或当前环境未启用设备配对。');
    } finally {
  ...
  const finishPairing = async () => {
    ...
    try {
      ...
      setPairingCode('');
      setPairingMessage(`设备已配对，短期会话将在 ${new Date(session.expiresAt * 1000).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })} 失效。`);
    } catch {
      setPairingMessage('配对失败：确认码无效、已过期或设备已被撤销。');
    } finally {
      setPairingBusy(false);
    }
  };
  ...
  return (
    <section className="axi-mobile-page axi-mobile-profile">
      <div className="axi-mobile-profile-card">
        ...
        <span className="axi-mobile-profile-card__identity">
          <strong>{user?.name || t('profile.guest')}</strong>
          <small>{user?.email || t('profile.account')}</small>
        </span>
      ...
      <div className="axi-mobile-setting-group axi-mobile-device-pairing">
        ...
        {deviceSession ? (
          <>
            <div className="axi-mobile-device-pairing__status"><strong>此设备已配对</strong><small>{deviceSession.deviceId}</small></div>
            <button type="button" className="axi-mobile-setting-row" onClick={() => { void clearMobileDeviceSession(); }}>
              <span>清除本机设备配对</span>
              <MobileIcon name="arrow-right" size={16} />
            </button>
          </>
          ...
          <>
            ...
            {!pairingStarted ? (
              <button type="button" className="axi-mobile-setting-row" disabled={pairingBusy} onClick={() => void beginPairing()}>
                <span>{pairingBusy ? '正在请求配对…' : '开始设备配对'}</span>
                <MobileIcon name="arrow-right" size={16} />
              </button>
            ) : (
              <div className="axi-mobile-device-pairing__confirm">
                <input value={pairingCode} inputMode="numeric" maxLength={6} onChange={(event) => setPairingCode(event.target.value.replace(/\D/g, ''))} placeholder="6 位确认码" />
                <button type="button" disabled={pairingBusy || pairingCode.length !== 6} onClick={() => void finishPairing()}>
                  {pairingBusy ? '正在确认…' : '确认配对'}
                </button>
              </div>
            )}
          </>
      ...
      {signedIn ? (
        <button type="button" className="axi-mobile-signout" onClick={() => { void clearMobileDeviceSession(); void logout(); navigate('/login'); }}>
