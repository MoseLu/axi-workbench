...
const Projects: React.FC = () => {
  ...
  return (
    ...
    >
      {error ? (
        <ControlPlaneState
          description="当前无法连接控制面；已登记项目会在连接恢复后显示。"
          title="项目目录暂不可用"
        />
      ) : isLoading ? (
        <ControlPlaneState description="正在从控制面读取项目目录。" loading title="正在同步项目" />
      ) : (
        <AxiTableGroup description={`显示 ${rows.length} 个已登记项目`} title="项目目录">
          <AxiTable
            columns={columns}
            data={rows}
            pagination={false}
