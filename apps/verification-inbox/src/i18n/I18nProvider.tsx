...
export type InboxLocale = 'zh-CN' | 'en-US';

export const INBOX_LOCALE_STORAGE_KEY = 'axi-verification-inbox.locale';

const DEFAULT_LOCALE: InboxLocale = 'zh-CN';

function isInboxLocale(value: unknown): value is InboxLocale {
  return value === 'zh-CN' || value === 'en-US';
}

function readStoredLocale(): InboxLocale {
  if (typeof window === 'undefined') return DEFAULT_LOCALE;
  try {
    const stored = window.localStorage.getItem(INBOX_LOCALE_STORAGE_KEY);
    return isInboxLocale(stored) ? stored : DEFAULT_LOCALE;
  } catch {
...
export interface I18nContextValue {
  locale: InboxLocale;
  setLocale: (next: InboxLocale) => void;
  /**
...
export interface I18nProviderProps {
  children: ReactNode;
  dictionaries?: Partial<Record<InboxLocale, Record<string, string>>>;
  defaultLocale?: InboxLocale;
}
...
export function I18nProvider({
  ...
  const [locale, setLocaleState] = useState<InboxLocale>(() => readStoredLocale() || defaultLocale);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(INBOX_LOCALE_STORAGE_KEY, locale);
    } catch {
      // localStorage may be disabled; the in-memory locale still works.
    }
  }, [locale]);

  const setLocale = useCallback((next: InboxLocale) => {
    setLocaleState(next);
  }, []);
