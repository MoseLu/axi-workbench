...
export WORKFLOW_URL="${WORKFLOW_URL:-${WORKFLOW_SERVICE_URL}}"
export NOTIFICATION_URL="${NOTIFICATION_URL:-${NOTIFICATION_SERVICE_URL}}"
...
if [[ -n "${GATEWAY_REDIS_URL+x}" ]]; then
  ...
  export GATEWAY_REDIS_URL="redis://127.0.0.1:16379/0"
fi
