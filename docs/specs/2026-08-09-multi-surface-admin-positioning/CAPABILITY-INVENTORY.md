...
| Web 账号与设置 | 通过 Gateway 调用 Identity、Notification、Platform Core；角色页读取和写入租户成员角色 | C / Workbench Web | 成员角色由 Platform Core 重新鉴权并审计；Mobile 不复制组织配置、批量和全量审计 |
| Web 运行状态 | 跨项目查看项目健康、运行环境与需要处理事项 | A / Workbench Web | 不提供通用摄像头扫码；登录确认和领域审批仍各自留在受控移动/身份流程 |
| Web 搜索与交接续办 | 注册路由搜索；Gateway 查询交接历史、读取/最终化服务端交接 | A/C / Workbench Web + Control Plane | 状态筛选、交接关联标识和 actor 审计贯穿 Mobile、Web 与最终动作 |
| Mobile 登录、概览、项目、工作区、搜索、交接历史 | Identity 会话与 Gateway 转发的已配对设备工作区/交接投影 | A/B / Workbench Mobile | 交接历史仅按 bearer 对应设备 actor 返回；显示更新时间、刷新、配对/权限/服务错误；搜索不再使用静态结果 |
