...
| Workflow Engine | `services/workflow-engine`（Python + FastAPI） | 仅接受 gateway 可信请求；工作流定义、执行认领与 Outbox event inbox 进入 PostgreSQL；执行器仍是进程内异步任务 |
...
- 三个专职服务已经进入 gateway/Helm 拓扑：workflow 与 notification 已具备 PostgreSQL schema、独立 migration Job、运行时账号、重启恢复和 Outbox event inbox 幂等边界；workflow 的事件触发编排、notification 的更多事件模板和 Kafka 适配仍待补。file 已具备 S3/MinIO 对象适配、PostgreSQL 元数据、迁移 Job、subject 隔离和短时预签名下载 URL；病毒扫描/缩略图等处理链仍待补齐，因此整个专职能力平面仍待集群级故障演练后才可称为最终生产完成。
