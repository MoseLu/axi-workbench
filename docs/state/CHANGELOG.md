...
- 2026-09-25：WFB-COMMIT-LEDGER-SYNC 接通 commit-ledger 数据源与持久化 ledger 的同步管道。根因：collector（`collect-all.mjs` + `repos-loader.mjs`）抓工作区 35 个项目 / 6,360 commits 完全没问题，但 `server.mjs` 启动后从未触发 `runSyncJob`，fs-watcher 默认关闭，`POST /commit-ledger/sync` 也没有 scheduler 兜底，导致 `.cache/commit-ledger/ledger.jsonl` 停留在 CL-018（2026-09-15）那次的 874 条 / 13 项目快照，混合 2 条 PERSISTENCE-TEST sentinel + 1 条 seed-1。修复：新增 [`services/control-plane/src/commit-ledger/scheduler.mjs`](/Volumes/code/workspace/workbench/axi-workbench/services/control-plane/src/commit-ledger/scheduler.mjs) 暴露 `startScheduler({runJob, jobs, intervalMs, startupMaxCommits, periodicMaxCommits, minIntervalMs, disabled, runStartupSync, logger})`，启动后立刻派一条 `startup` 任务，再按默认 15 min 周期触发 `interval` 任务，遇到上一条 scheduler-owned 任务仍在 in-flight 时跳过当前 tick；`POST /commit-ledger/sync` 路径保持不变，作为手动旁路。`server.mjs:1162-1187` 在 `initCommitLedgerStore()` 之后立即起 scheduler，env override 为 `AXI_COMMIT_LEDGER_SYNC_DISABLED=1`（关）/ `AXI_COMMIT_LEDGER_SYNC_INTERVAL_MS`（改 tick）/ `AXI_COMMIT_LEDGER_STARTUP_MAX_COMMITS` / `AXI_COMMIT_LEDGER_SYNC_MAX_COMMITS`；SIGTERM/SIGINT 复用同一个 `shutdownCommitLedger` 钩子同时停 scheduler + fs-watcher。fs-watcher 语义保持不变（opt-in via `AXI_COMMIT_LEDGER_FS_WATCHER=enabled`），但日志改成 "scheduler running as baseline; fs_watcher opt-in"。新增 [`services/control-plane/src/commit-ledger/scheduler.test.ts`](/Volumes/code/workspace/workbench/axi-workbench/services/control-plane/src/commit-ledger/scheduler.test.ts) 9/9 通过：覆盖 startup 立即派发、`runStartupSync=false` 跳过、interval tick 节奏、in-flight 重叠跳过、stop() 终止、disabled 旁路、`runNow` 手动触发 / 与 in-flight 互斥、`status()` 生命周期。`services/control-plane/src/commit-ledger/index.mjs` 同步导出 `startScheduler`。end-to-end 验证：手工跑一次 startup sync（`/tmp/prod-sync-check.mjs`）后 `ledger.jsonl` 由 877 行 / 13 项目 / 874 真实 + 2 sentinel + 1 seed 涨到 6,364 行 / 36 项目 / 6,361 真实 + 2 sentinel + 1 seed，新增 23 个项目（axi-pet-desktop / axi-rules / axi-registry / axi-ui / axi-skills / blinko / cockpit-tools / comfyui / dbskill / image2prompt / observability / opencodex / pelagic / sports-management / story-graph / sub2api / voice-assistant-on-device-speech-recognition / 三个 axi-workbench-*-dist / axi-runtime / axi-sync / axi-video-downloader），axi-soul-world 由 50 涨到 226。按 owner 要求 3 条 sentinel/seed 保留不动（idempotency 不删除历史），被 6,361 条真实数据稀释到 0.05%，UI 上基本不可见。回归确认：`persistence.test.ts` 30/30、`integration.test.ts` 22/22；`commit-ledger-http.test.mjs` 8/10 通过，2 个失败 (`expected 200 or 500, got 202` + `/workbench/axi-workbench` 路径不匹配) 为预存脏状态（在 HEAD 上同样失败，与本次改动无关）。证据：[`scheduler.mjs`](/Volumes/code/workspace/workbench/axi-workbench/services/control-plane/src/commit-ledger/scheduler.mjs)、[`scheduler.test.ts`](/Volumes/code/workspace/workbench/axi-workbench/services/control-plane/src/commit-ledger/scheduler.test.ts)、[`server.mjs:1150-1215`](/Volumes/code/workspace/workbench/axi-workbench/services/control-plane/src/server.mjs)、[`commit-ledger-architecture.md § Sync Pipeline`](/Volumes/code/workspace/workbench/axi-workbench/docs/commit-ledger-architecture.md)、[`commit-ledger-status.md` CL-019b 行](/Volumes/code/workspace/workbench/axi-workbench/docs/commit-ledger-status.md)。

- 2026-09-20：WFB-BASE-MERGE 完成 baseline 三项目并入 `axi-workbench`。将原独立仓库 `products/axi-artboard`（agent node-picker + 自研 vite plugin）、`products/ai-resource-orchestration`（gateway + 8 workspace packages + 浏览器 workbench，gateway 服务在 `127.0.0.1:8787`，浏览器 workbench 在 `:5177`）、`projects/axi-docs`（workspace docs hub + 知识图谱 + MCP document bus）整体迁入 `axi-workbench` monorepo 作为 hosted sub-apps + workspace packages。新增 `apps/axi-artboard/`、`apps/resource-orchestration/`、`services/resource-gateway/`、`apps/axi-docs/`、`packages/artboard-vite-plugin/`（`@axi/artboard-vite-plugin`，AST 源戳 + 感知 HTTP 中间件），以及 8 个 `@axi/resource-*` + `@axi/gateway-contracts` workspace packages（`@axi/resource-{adapters,orchestrator,memory,session,config,api-docs,gateway,orchestration-workbench}`）。`axi-artboard` 的 vite plugin 改写为 `@babel/parser` + `@babel/traverse`（不再依赖 Vite 8 `parseAst`），全部栈对齐到 React 18.3 + Vite 5.4 + TS 5.3 + Vitest 3.2 + JestDOM 7（workbench root baseline）；`apps/resource-orchestration` 从 `link:` 切到 `workspace:*`，消除 workbench 内 React 18 + 19 双实例风险；`apps/axi-docs/app` 删除原独立 `pnpm-workspace.yaml` + `pnpm-lock.yaml` 并将其 pnpm.overrides 提升到 workbench 根，workspace `pnpm-workspace.yaml` 显式登记 `apps/axi-docs/app` importer（`apps/*` glob 不匹配嵌套子目录）。`devsvc-dashboard/config/axi-apps.json` + `axi-resources.json` + `scripts/axi-app-host.mjs` 注册 `axi-artboard` / `axi-resource-orchestration` / `axi-resource-gateway` / `axi-docs` 四个新 hosted app/gateway；`scripts/check-workbench-boundaries.mjs` 豁免收紧到 `apps/axi-docs/{app/scripts,app/src/lib,app/src/components,infra}/**`；`services/control-plane/src/commit-ledger/collect-all.mjs` 更新三个项目的 PATH_MAP。`pnpm verify:ci` 通过；`workspace-project handoff-check axi-workbench` 返回 `{"ok":true,"status":"verified","score":10,"errors":0,"warnings":0}`；`node scripts/check-workbench-boundaries.mjs` 仅剩 3 个 pre-existing violations（commit-ledger/test-loader），新增 0 个；8 个 packages typecheck 全过 + 67 test files / 1042 tests passed；apps/axi-artboard typecheck 0 errors + 10 files / 68 tests passed；apps/resource-orchestration typecheck 0 errors + 10 files / 105 tests passed；apps/axi-docs/app `tsc --noEmit` 退出码 0；apps/workbench 测试从 baseline 37 passed / 4 failed → 39 passed / 2 failed（pre-existing react-router 7 SSR 兼容性问题，与本次合并无关）。11 个功能 commit 链：`f0ac428b feat(workbench): migrate axi-artboard (vite plugin + app, React 18 / Vite 5.4 / TS 5.3)`、`cb742c10 feat(workbench)!: merge ai-resource-orchestration (apps + services + 8 @axi/* packages)`、`00ce02bf feat(workbench): migrate axi-docs into apps/axi-docs`、`1e24f674 chore(devsvc-dashboard): register workbench-hosted apps + gateway`、`f3b3c3f3 chore(workbench): exempt apps/axi-docs from boundary check + update commit-ledger path map`、`23fd6e55 chore(apps): replace link: specifiers with workspace:* in apps/resource-orchestration`、`edb49982 chore(apps): drop apps/axi-docs/app inner pnpm-workspace.yaml + lockfile`、`13388871 chore(workbench): narrow apps/axi-docs boundary-check exemption to scripts/lib/components/infra`、`5df9ae1f chore(workbench): lift apps/axi-docs/app inner overrides into root + register importer`、`0177e66c chore(apps): fix apps/resource-orchestration vitest 4.x type + test failures`、`f5c66a50 chore(apps): fix apps/axi-artboard vitest run mode + jest-dom 7`。外部 follow-up（非本次合并范围）：(1) `WORKSPACE_INDEX.md` + `workspace.graph.json` + `foundation/workspace-governance/workspace.json` 删除三个已合并项目的 admission 条目（本次在 `/Volumes/code/workspace/` 根直接修改了前两个，第三个属于独立 governance 仓需要单独提交）；(2) `apps/workbench` 的 2 个 pre-existing react-router 7 SSR test failure（`legal-ssr-entry.test.tsx` + `DesktopSettingsPage.test.tsx`）；(3) 远端仓库 `products/axi-artboard`、`products/ai-resource-orchestration`、`projects/axi-docs` 标记为 archived。证据：[WORKSPACE_INDEX.md](/Volumes/code/workspace/WORKSPACE_INDEX.md)、[workspace.graph.json](/Volumes/code/workspace/workspace.graph.json)、`pnpm verify:ci` 输出、`workspace-project handoff-check axi-workbench` 输出、`pnpm -r --filter @axi/{gateway-contracts,resource-*,artboard-vite-plugin} test` 输出、`pnpm --dir apps/{axi-artboard,resource-orchestration} test` 输出、`apps/axi-docs/app` `pnpm exec tsc --noEmit` 输出。

### Changed

- 2026-09-15：WFB-REG-001 收紧 Resource Registry 生命周期状态类型。`AxiResource.status` 从 `ResourceLifecycleStatus | string` 改为严格 `ResourceLifecycleStatus`，`ResourceLifecycleStatus` 改为基于 `RESOURCE_LIFECYCLE_STATUSES` 常量数组的派生 union，确保类型与运行时值同源。新增 `normalizeResourceStatus(value: unknown): ResourceLifecycleStatus`：已知字符串原样返回并保持引用相等，非已知值（未知字符串、空串、大小写不一致、非字符串、`undefined`、`null`、数字、对象、数组、布尔）一律归一化为 `'path-found'` 并通过 `console.warn` 输出 `[axi-resources] Unknown ResourceLifecycleStatus …; normalizing to "path-found"`，已知状态不触发警告，SSR / Worker 等 `console` 缺失环境也不会抛错。`listAxiResources` 在 API 解析边界对每条资源调用 `normalizeResourceStatus`，未知的 `status` 不再通过类型逃逸进入页面、StatusChip、搜索或详情渲染。新增 `scripts/axi-resources.test.mjs` 覆盖六个归一化分支（已知透传、元组覆盖、未知字符串、非字符串、已知零警告、重复未知值每次警告）。新增 `scripts/test-loader.mjs` 通过 `--import` 注册 tsx 钩子，使 `--test scripts/*.test.mjs` 能加载 TypeScript 源；`package.json` 增加 `test:legacy` 兜底保留纯 JS 测试入口。`pnpm --dir apps/devsvc-dashboard typecheck` 退出码 0；新增 6/6 通过，整体测试 99/102 通过（3 个 `resource-metadata.test.mjs` 失败为 `WFB-REG-002` 既有阻塞，与 WFB-REG-001 无关）。证据：[axiResources.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/axi-resources/axiResources.ts)、[axi-resources.test.mjs](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/scripts/axi-resources.test.mjs)、[test-loader.mjs](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/scripts/test-loader.mjs)、[package.json](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/package.json)。
- 2026-09-15：WFB-NAV-001 接通真实 Shell 用户角色。`AuthUser.role` 改为必填字段，`Shell.tsx:89` 直接传入 `user.role`，移除 `user.role || getUserRole()` 的隐式 fallback；新增 `resolveRoleForUsername` 让登录和 `readStoredAuth` 都写入 `AuthUser.role`。新增 `scripts/navigation-roles.test.mjs`，覆盖 user / developer / admin 三角色 × hidden / deferred / admin 三种 visibility 的导航断言矩阵，以及 audience 门禁、Shell 注入、AuthUser 类型不变量。`pnpm --dir apps/devsvc-dashboard typecheck` 退出码 0；新测试 10/10 通过。`getUserRole` 的 fallback 语义保留给配置和初始化路径。证据：[Shell.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/app-shell/Shell.tsx)、[auth.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/auth/auth.ts)、[useAuthState.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/auth/useAuthState.ts)、[navigation-roles.test.mjs](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/scripts/navigation-roles.test.mjs)。
- 2026-09-15：WFB-SEC-001 完成私有资源暴露测试。`makeGlobalSearchItems` 新增 `userRole` 参数并透传到 `translateNavGroups`，全局搜索对 `user` / `developer` 隐藏 hidden / admin / deferred 资源；`app-registry.tsx` 新增 `getNavItemsByRole` 辅助函数作为角色导航的 canonical 入口；`AxiResourcesPage.tsx` 导出 `redactResourceForRole` 与 `canRoleAccessResource` 纯函数：普通用户/开发者渲染时 `ownerPath` 被脱敏，`user` 额外脱敏 `evidenceLink` / `docsRoute`；直接访问 hidden 路由返回授权错误（access-denied 标识），`deferred` 对所有角色（含 admin）拒绝访问，`hidden` 仍是 admin 唯一逃生口。`Shell.tsx:275-276, 340` 把 `userRole={user.role}` 显式传入 `AxiResourcesPage` 与 `GlobalSearchBox`。新增 `scripts/security-private-resources.test.mjs` 15/15 通过：覆盖三角色导航过滤、全局搜索过滤、详情页 ownerPath / evidenceLink / docsRoute 字段脱敏、序列化视图不含绝对路径或未授权 URL、hidden 路由授权门、deferred 全角色拒绝，以及 React 源代码中三个 surface 同步注入了 `userRole` 链路的源级不变量。`pnpm --dir apps/devsvc-dashboard typecheck` 退出码 0；测试 100/102 通过（2 个 `resource-metadata.test.mjs` 失败为 `WFB-REG-002` 既有阻塞，与 WFB-SEC-001 无关）。证据：[app-registry.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/app-registry.tsx)、[AxiResourcesPage.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/axi-resources/AxiResourcesPage.tsx)、[GlobalSearchBox.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/search/GlobalSearchBox.tsx)、[Shell.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/app-shell/Shell.tsx)、[security-private-resources.test.mjs](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/scripts/security-private-resources.test.mjs)。
- 2026-09-15：WFB-HOST-001 修复 degraded state 测试。`scripts/axi-app-host.test.mjs` 中 "startApp reports degraded state when page is accessible but readiness endpoint never returns 200" 用例的 startCommand 由 `sleep 30`（无端口绑定，导致 healthCheck 返回 `unavailable`，触发 `finalizeStartFailure` → `error`）改为 Node 自带 HTTP 代理转发到 fake upstream 端口的命令，使 `waitForReady` 的 `classifyHealth` 能正确探测到 `page-only` 状态（health 200 / readiness 503），从而触发 `attemptStart` 中 `result.status === "page-only"` 分支并收敛到 `degraded` 状态。`pnpm --dir apps/devsvc-dashboard test` 退出码 0，110/110 全部通过（含 axi-app-host 17/17、security-private-resources 15/15、navigation-roles 10/10、axi-resources 6/6、verification-persistence、resource-metadata、scan-runtime-paths、drift-check、workspace-resource-registry 等）；`pnpm --dir apps/devsvc-dashboard typecheck` 退出码 0。证据：[axi-app-host.test.mjs:300-365](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/scripts/axi-app-host.test.mjs)、[TODO.md §0 `WFB-HOST-001`](/Volumes/code/workspace/workbench/axi-workbench/docs/specs/2026-09-14-workspace-foundation-binding/TODO.md)。
- 2026-09-15：WFB-REL-001 真正修复 Dashboard production build 与 Dashboard 白屏。两步根因：(1) 构建侧 `axi-core` 单 chunk 1,477,646 字节超过 `enforceMaxChunkSize` 1 MB 门禁；(2) 运行时侧 monorepo 内多 React 版本（`react@19.2.6` 与 `react-dom@19.2.8` 由 `verification-inbox` `^19.1.0` 间接解析）触发 React 19 `Incompatible React versions` 异常，`createRoot().render()` 在 main.tsx 顶层抛出导致 `<div id="root">` 永远为空。修复：在 [`apps/devsvc-dashboard/vite.config.ts`](../../../../apps/devsvc-dashboard/vite.config.ts) `chunkVendor()` 中把 `lucide-react` 图标按子模块拆为 `axi-core-icons-1..5` 等多个 chunk，保留 `maxChunkSizeBytes = 1_000_000` 与 `enforceMaxChunkSize()` 严格门禁（**未放宽**）；在 [`apps/devsvc-dashboard/package.json`](../../../../apps/devsvc-dashboard/package.json) 把 `react` 与 `react-dom` 锁到精确 `19.2.6`，并配合 `pnpm.overrides` 防止 transitive hoist。修复后：`pnpm --dir apps/devsvc-dashboard build` 退出码 0、最大 JS chunk `antd-runtime-B0hhTMPW.js` = 631,110 字节、所有 19 个 JS chunk 均 ≤ 631 kB、19 CSS chunk 全部 ≤ 70 kB；`pnpm --dir apps/devsvc-dashboard test` 110/110 通过；`pnpm --dir apps/devsvc-dashboard typecheck` 0 errors；启动 `pnpm --dir apps/devsvc-dashboard dev` 后 Chrome CDP 验证 `document.getElementById("root").innerHTML` 已渲染为完整 LoginPage（含 `login-screen`/`login-panel`/`login-form` 与用户名密码字段），登录 admin/admin 后跳转 `/overview` 显示 20 项目 / 9 健康 / 健康率 45%。证据：[vite.config.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/vite.config.ts)、[package.json](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/package.json)、`pnpm --dir apps/devsvc-dashboard build` 实际输出、Chrome DevTools Protocol console 捕获、`TODO.md` §0 `WFB-REL-001` 与 `WFB-QA-001-REDO` 行。
- 2026-09-15：WFB-QA-FIX 修复 `/axi-resources` 路由黑屏。根因：后端 `/api/axi/resources` 返回 `resources: {}`（对象）而非 `resources: []`（数组），前端 `data?.resources || []` 因对象是 truthy 而绕过 fallback，导致 `resources.filter is not a function` 异常抛出后 React 卸载整个树。修复：[`axiResources.ts`](../../../../apps/devsvc-dashboard/src/features/axi-resources/axiResources.ts) 第 149 行 `body.resources || []` 改为 `Array.isArray(body.resources) ? body.resources : []`；[`AxiResourcesPage.tsx`](../../../../apps/devsvc-dashboard/src/features/axi-resources/AxiResourcesPage.tsx) 第 503 行同样收口。Chrome CDP 实测 `/axi-resources` 后 `document.getElementById("root").innerHTML.length = 49352`、`hasMain = true`、`hasTable = true`，资源表格完整渲染。同时 WFB-ROLE-FIX 把 `readStoredAuth` 从「仅接受 adminUsername」扩展为「接受任意在 `DEV_LOGIN_ACCOUNTS` 表中存在或解析得 role 的 username」，新增 `validateLoginCredentials` / `listDevLoginAccounts`；`LoginPage` 新增三角色账号面板（admin/developer/user，点击即可填充表单）。`useAuthState` 新增 `storage` / `axi-auth-sync` 事件监听以同步外部 localStorage 修改。WFB-ASSET-FIX 在 `vite.config.ts` `server.fs.allow` 显式列入 `foundation/axi-ui` 路径，消除 dev server `[plugin devsvc-max-chunk-size]`/`outside of Vite serving allow list` 对 `@axi/core/dist/assets/axi-dango-family.png` 的告警；生产构建 `pnpm --dir apps/devsvc-dashboard build` 仍把 `axi-dango-family-B1rihLlo.png`（1.9 MB）正确打包到 `dist/assets/`。WFB-QA-001 1440px Chrome headless 实测：admin/developer/user 三种角色 × `/overview`、`/axi-resources`、`/axi-resources/axi-registry`、`/services` 共 12 张截图保存于 `/tmp/wfb-qa-screenshots/{role}-{route}.png`（文件大小 53k–160k 字节，因页面内容不同而各异），证明每种角色渲染独立视图；隐藏资源 `/axi-resources/axi-registry`（`visibility: hidden`）由 `WFB-SEC-001` 的授权门逻辑拒绝非 admin 角色。`pnpm --dir apps/devsvc-dashboard test` 110/110 通过；`pnpm --dir apps/devsvc-dashboard typecheck` 0 errors；`pnpm --dir apps/devsvc-dashboard build` 退出码 0。证据：[axiResources.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/axi-resources/axiResources.ts)、[AxiResourcesPage.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/axi-resources/AxiResourcesPage.tsx)、[auth.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/auth/auth.ts)、[useAuthState.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/auth/useAuthState.ts)、[LoginPage.tsx](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/src/features/auth/LoginPage.tsx)、[vite.config.ts](/Volumes/code/workspace/workbench/axi-workbench/apps/devsvc-dashboard/vite.config.ts)、`/tmp/wfb-qa-screenshots/` 截图。
- 2026-09-14：完成工作区基础项目与 Axi Workbench 绑定整改核心功能：AxiResource 6态生命周期状态（registered/path-found/verified/stale/failed/missing）、computeLifecycleStatus() 验证逻辑、audience 权限过滤 + menuGroup 动态分组、文档入口列 + 失败状态 tooltip、executionBoundary 配置（axi-docs/axi-agent）、visibility + menuGroup 配置完善、drift-check.mjs P2自动化检查脚本。workspace-project validate、pnpm check:boundaries、Dashboard typecheck/tests 21/21、axi-ui typecheck、axi-skills verify errors=0、axi-registry health 全部通过。commit `90fc786d`。
...
- Documentation ownership is now explicit for `/Volumes/code/workspace/workbench/axi-workbench`.
